...
class DataManager(object):
    ...
    def get_dataset(self,
        ...
        if self.debug and len(data) > debug_data_num:
            debug_idx = np.random.choice(np.arange(len(data)),
                                         debug_data_num,
                                         replace=False)
            data, targets = data[debug_idx], targets[debug_idx]
    ...
    def get_finetune_dataset(self,
        ...
        if type == "ratio":
            new_num_tot = int(old_num_tot * (total_classes - known_classes) /
                              known_classes)
        elif type == "same":
            new_num_tot = old_num_tot
        else:
            assert 0, "not implemented yet"
        new_num_average = int(new_num_tot /
                              max(total_classes - known_classes, 1))
        ...
        if self.debug and len(val_data) > debug_data_num:
            debug_idx = np.random.choice(np.arange(len(val_data)),
                                         debug_data_num,
                                         replace=False)
            val_data, val_targets = val_data[debug_idx], val_targets[debug_idx]
    ...
    def get_dataset_with_split(self,
        ...
        if self.debug and len(train_data) > debug_data_num:
            debug_train_idx = np.random.choice(np.arange(len(train_data)),
                                               debug_data_num,
                                               replace=False)
            train_data, train_targets = train_data[
                debug_train_idx], train_targets[debug_train_idx]
        if self.debug and len(val_data) > debug_data_num:
            debug_val_idx = np.random.choice(np.arange(len(val_data)),
                                             debug_data_num,
                                             replace=False)
            val_data, val_targets = val_data[debug_val_idx], val_targets[
                debug_val_idx]

        return DummyDataset(train_data, train_targets, trsf,
                            self.use_path), DummyDataset(
    ...
    def _setup_data(self, dataset_name, shuffle, seed):
        ...
        # Map indices
        self._train_targets = _map_new_class_index(self._train_targets,
                                                   self._class_order)
