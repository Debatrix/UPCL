...
class CosineIncrementalNet(BaseNet):
    ...
    def update_fc(self, nb_classes, task_num):
        ...
        if self.fc is not None:
            if task_num == 1:
                ...
                fc.fc1.weight.data[:
                                   prev_out_features1] = self.fc.fc1.weight.data
                fc.fc1.weight.data[
                    prev_out_features1:] = self.fc.fc2.weight.data
                fc.sigma.data = self.fc.sigma.data
    ...
    def generate_fc(self, in_dim, out_dim):
        if self.fc is None:
            ...
            fc = SplitCosineLinear(in_dim, prev_out_features,
                                   out_dim - prev_out_features, self.nb_proxy)
    ...
    def forward(self, x):
        ...
        if self.bias_correction:
            ...
            for i, layer in enumerate(self.bias_layers):
                logits = layer(logits, sum(self.task_sizes[:i]),
                               sum(self.task_sizes[:i + 1]))
            out["logits"] = logits
...
class DERNet(nn.Module):
    ...
    def update_fc(self, nb_classes):
        ...
        if self.fc is not None:
            ...
            weight = copy.deepcopy(self.fc.weight.data)
            bias = copy.deepcopy(self.fc.bias.data)
            fc.weight.data[:nb_output, :self.feature_dim -
                           self.out_dim] = weight
            fc.bias.data[:nb_output] = bias
...
class SimpleCosineIncrementalNet(BaseNet):
    def __init__(self, args, pretrained):
        super().__init__(args, pretrained)
        self.use_sigma = args.get('use_sigma',True)
    ...
    def update_fc(self, nb_classes, nextperiod_initialization=None):
        ...
        if self.fc is not None:
            ...
            if nextperiod_initialization is not None:
                ...
                weight = torch.cat([
                    weight,
                    torch.zeros(nb_classes - nb_output,
                                self.feature_dim).to(weight.device)
                ])
    ...
    def generate_fc(self, in_dim, out_dim):
        fc = CosineLinear(in_dim, out_dim,sigma=self.use_sigma)
        return fc
...
class BEEFISONet(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.old_fc is None:
            ...
            fc_weight = torch.cat([
                self.old_fc.weight,
                torch.zeros(
                    (new_task_size, self.feature_dim - self.out_dim)).to(
                        self.old_fc.weight.device)
            ],
                                  dim=0)
            ...
            for i in range(len(self.task_sizes) - 2, -1, -1):
                new_fc_weight = torch.cat([
                    *[
                        self.biases[i](
                            ...
                            bias=False) for _ in range(self.task_sizes[i])
                    ], new_fc_weight
                ],
                                          dim=0)
                new_fc_bias = torch.cat([
                    *[
                        self.biases[i](
                            self.backward_prototypes.bias[i].unsqueeze(0),
                            bias=True) for _ in range(self.task_sizes[i])
                    ], new_fc_bias
                ])
            fc_weight = torch.cat([fc_weight, new_fc_weight], dim=1)
            fc_bias = torch.cat([
                self.old_fc.bias,
                torch.zeros(new_task_size).to(self.old_fc.weight.device)
            ])
            fc_bias = +new_fc_bias
            ...
            out["train_logits"] = features[:, -self.
                                           out_dim:] @ new_fc_weight.permute(
                                               1, 0) + new_fc_bias
        out.update({
            "eval_logits":
            out["logits"],
            "energy_logits":
            self.forward_prototypes(features[:, -self.out_dim:])["logits"]
        })
        return out
    ...
    def update_fc_after(self):
        if self.old_fc is not None:
            ...
            old_fc.weight.data = torch.cat([old_fc.weight.data, new_fc_weight],
                                           dim=1)
            old_fc.bias.data = torch.cat([
                self.old_fc.bias.data,
                torch.zeros(new_task_size).to(self.old_fc.weight.device)
            ])
...
class AdaptiveNet(nn.Module):
    def __init__(self, args, pretrained):
        ...
        self.TaskAgnosticExtractor, _ = get_convnet(
            args, pretrained)  #Generalized blocks
    ...
    def forward(self, x):
        ...
        features = torch.cat(backbones_features, 1)
        ...
        out.update({
            ...
            "backbones_features": backbones_features
        })
        return out
        '''
        {
    ...
    def load_checkpoint(self, args):
        ...
        pretrained_base_dict = {
            k: v
            for k, v in model_dict.items() if k in base_state_dict
        }

        pretrained_adap_dict = {
            k: v
            for k, v in model_dict.items() if k in adap_state_dict
        }
