...
class BaseLearner(object):
    ...
    def eval_task(self, save_conf=False):
        output = self._eval_cnn(self.test_loader)
        y_pred, y_true = output['y_pred'], output['y_true']
        cnn_accy = self._evaluate(y_pred, y_true)
        cm = confusion_matrix(y_true, y_pred[:, 0])
        cnn_accy['cm'] = cm
        cnn_accy['y_true'] = y_true
        cnn_accy['logits'], cnn_accy['features'] = output['logits'], output['features']
        try:
            cnn_accy['prototype'] = self._network.fc.weight.clone().detach().cpu().numpy()
        except:
            cnn_accy['prototype'] = None
    ...
    def _eval_cnn(self, loader):
        ...
        for _, (_, inputs, targets) in enumerate(loader):
            ...
            with torch.no_grad():
                ...
                _logits = outputs["logits"]
                _features= outputs.get('features',torch.zeros(0))
            predicts = torch.topk(_logits,
                                  k=self.topk,
                                  dim=1,
                                  largest=True,
        ...
        ret['y_pred'] = np.concatenate(y_pred) # [N, topk]
        ret['y_true'] = np.concatenate(y_true)
        ret['logits'] = np.concatenate(logits)
        ret['features'] = np.concatenate(features)
    ...
    def _construct_exemplar(self, data_manager, m):
        ...
        for class_idx in range(self._known_classes, self._total_classes):
            ...
            exemplar_targets = np.full(_m, class_idx)
            self._data_memory = (np.concatenate(
            ...
            self._class_means[class_idx, :] = mean
        if len(less_than_m) > 0:
            logging.info("Classes with less than {} exemplars: {}".format(
                m, less_than_m))
