...
from utils.toolkit import count_parameters, loss_counter, target2onehot, tensor2numpy
...
class MEMO(BaseLearner):
    ...
    def _train(self, train_loader, test_loader):
        ...
        if self._cur_task == 0:
            ...
            if not self.args['skip']:
                ...
                cur_test_acc = self._compute_accuracy(self._network,
                                                      self.test_loader)
                logging.info(
                    f"Loaded_Test_Acc:{load_acc} Cur_Test_Acc:{cur_test_acc}")
        else:
            optimizer = optim.SGD(filter(lambda p: p.requires_grad,
                                         self._network.parameters()),
                                  lr=self.args['lrate'],
            ...
            if self.args['scheduler'] == 'steplr':
                ...
                scheduler = optim.lr_scheduler.CosineAnnealingLR(
                    optimizer=optimizer, T_max=self.args['t_max'])
            else:
                raise NotImplementedError
    ...
    def _init_train(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                total += len(targets)
                backbones_features_norm = [
                    x.norm(p=2, dim=1).mean(0) for x in backbones_features
                ]
                epoch_norm.update(backbones_features_norm)
            ...
            if epoch % 5 == 0:
                ...
                info = 'Task {}, Epoch {}/{} => Loss {:.3f}, Train_accy {:.2f}, Test_accy {:.2f}, Backbone Norm [{}]'.format(
    ...
    def _update_representation(self, train_loader, test_loader, optimizer,
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                backbones_features = outputs.get('backbones_features', [])
            ...
            if epoch % 5 == 0:
                test_acc = self._compute_accuracy(self._network, test_loader)
                info = 'Task {}, Epoch {}/{} => Loss {:.3f}, Loss_clf {:.3f}, Loss_aux  {:.3f}, Train_accy {:.2f}, Test_accy {:.2f}, Backbone Norm [{}]'.format(
                    self._cur_task, epoch + 1, self.args["epochs"],
                    losses / len(train_loader), losses_clf / len(train_loader),
    ...
    def save_checkpoint(self, test_acc):
        ...
        torch.save(save_dict, "{}_{}.pkl".format(checkpoint_name,
                                                 self._cur_task))
